...
const String mi_remote_ver = "MI_REMOTE : V00.000.001";
...
//const uint16_t kIrLed = 4;//23; // 4 D2;  //Recommended: ESP8266 = 4 (D2). ESP32 = 23. LED GND to GPIO , +ve connect to 3.3v pin

IRsend irsend(IR_LED_PIN);  // kIrLed Set the GPIO to be used to sending the message.
...
void MI_BACK()
{
  Serial.println("MI_TV_BACK");
  irsend.sendRaw(MI_TV_BACK, 167, 38);  // Send a raw data capture at 38kHz.
}
void MI_VOL_DOWN()
{
  Serial.println("MI_TV_VOLUME_MINUS");
  irsend.sendRaw(MI_TV_VOLUME_MINUS, 167, 38);  // Send a raw data capture at 38kHz.
}
void MI_VOL_UP()
{
  Serial.println("MI_TV_VOLUME_PLUS");
  irsend.sendRaw(MI_TV_VOLUME_PLUS, 167, 38);  // Send a raw data capture at 38kHz.
}
void MI_OK()
{
...
void MI_LEFT()
{
  Serial.println("MI_TV_SCROLL_LEFT");
  irsend.sendRaw(MI_TV_SCROLL_LEFT, 167, 38);  // Send a raw data capture at 38kHz.
}
void MI_RIGHT()
{
...
void MI_UP()
{
  Serial.println("MI_TV_SCROLL_UP");
...
void MI_DOWN()
{
  Serial.println("MI_TV_SCROLL_DOWN");
...
void Tata_sky_send() // example for tatasky setup box
{
  Serial.println("NINE");
